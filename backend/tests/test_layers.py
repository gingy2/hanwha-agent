...
# 파일 최상단에서 import하는 모듈 이름 한데 모으기
...
# 모듈이 크 패키지에 속하는지 여부 판단

def _is(name: str, prefix: str) -> bool:
    return name == prefix or name.startswith(prefix + ".")

# 한 계층 안에서 금지된 import를 하는 자리 모으기 (?먼말임?)
def _violations(
    layer: str,
    forbidden: tuple[str, ...],
    allowed: tuple[str, ...] = (),
...
# 모델은 최하위 계층이라 hanwha-app 내의 다른 계층을 모른다. 다른 계층을 역으로 부르는지 검사
...
# router에서 한 층 건너뛰어 DB 계층을 직접 부르지 않게 검사
...
# service에서 API router를 import하지 않게 검사
...
# repository는 DB만 다룬다. service 또는 API router를 import하지 않게 검사
